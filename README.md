...
### □ 開発背景
実務で利用されるシステムを想定し、「堅牢な認証システム」と「データベース性能の最適화」を重点的に開発しました。ユーザーの状態遷移（認証前、有効、退会済）に伴うアクセス制御と、JPAによる効率的なデータ照会を実装しています。
...
### □ 主な機能 (Core Features)
1. ユーザー状態管理: ACTIVE, INACTIVE(未認証), WITHDRAWN(退会済)によるアクセス制御
...
5. 활동 관리: 마이페이지를 통해 본인이 작성한 게시글 및 참여도 확인

---

## ■ 技術的解決 (Technical Problem Solving)

### 01. JPA N+1 問題の解決 (N+1 문제 해결)
...
### 02. セキュリティハンドラーによる例外処理 (보안 핸들러 처리)
- **課題**: ログイン失敗時、理由（パスワード不一致、退회済、未認証）をユーザーが特定できず利반性が低下
- **解決**: `AuthenticationFailureHandler` を拡張し、例外の種類に応じたエラーメッセージを返却。また、`SuccessHandler` で退会アカウントのセッションを即時破棄

### 03. オブジェクト指向に基づいたドメイン設計 (객체지향 도메인 설계)
- **課題**: サービス層にロ직이 集中し、コードの保守性が低下
...
- **PostLike**: 重복防止のため、UserとPostの組み合わせにユニーク制約を適用
